...
resource "aws_iam_policy" "capstone_cloudwatch_policy" {
...
resource "aws_iam_policy" "capstone_dynamodb_policy" {
...
resource "aws_iam_policy" "capstone_rekognition_policy" {
...
resource "aws_iam_role_policy_attachment" "capstone_cloudwatch_attach" {
  role       = aws_iam_role.role_for_lamda_functions_tf.name
  policy_arn = aws_iam_policy.capstone_cloudwatch_policy.arn
}
...
resource "aws_iam_role_policy_attachment" "dynamodb_attach" {
  role       = aws_iam_role.role_for_lamda_functions_tf.name
  policy_arn = aws_iam_policy.capstone_dynamodb_policy.arn
}

resource "aws_iam_role_policy_attachment" "rekognition_attach" {
  role       = aws_iam_role.role_for_lamda_functions_tf.name
  policy_arn = aws_iam_policy.capstone_rekognition_policy.arn
}
...
#   role        = aws_iam_role.capstone-lambda-role.name
    ...
    Statement = [
      {
        Effect   = "Allow"
        Action   = "s3:PutObject"
        Resource = "arn:aws:s3:::studentimages-tf/*"
      }
    ]
  })
}
    ...
    Statement = [
      {
        Effect   = "Allow"
        Action   = "s3:PutObject"
        Resource = "arn:aws:s3:::classphotos-tf/*"
      }
    ]
  })
}