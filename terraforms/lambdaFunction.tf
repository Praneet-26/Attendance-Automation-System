locals {
  python_files = replace(path.cwd,"terraforms","lambda")
}

data "archive_file" "zip_the_student_registration_code" {
  type        = "zip"
  source_dir  = "${local.python_files}/register_student/"
  output_path = "${local.python_files}/register_student/handler.zip" 
}

data "archive_file" "zip_the_student_authentication_code" {
  type        = "zip"
  source_dir  = "${local.python_files}/process_class_photo/"
  output_path = "${local.python_files}/process_class_photo/handler.zip" 
}
...
resource "aws_lambda_permission" "s3_invoke_permission" {
  ...
  source_arn    = aws_s3_bucket.new-student-registration-tff.arn
}
